...
const IngredientTooltip = ({ name }) => {
  ...
  const exactHighMatch = Object.keys(highRiskDescriptions).find(
    key => key.toLowerCase() === normalizedName
  );
  const exactMediumMatch = Object.keys(mediumRiskDescriptions).find(
    key => key.toLowerCase() === normalizedName
  );

  // If no exact match, try partial matches
  const partialHighMatch = !exactHighMatch && Object.keys(highRiskDescriptions).find(
    key => key.toLowerCase().includes(normalizedName) || normalizedName.includes(key.toLowerCase())
  );
  const partialMediumMatch = !exactMediumMatch && Object.keys(mediumRiskDescriptions).find(
    key => key.toLowerCase().includes(normalizedName) || normalizedName.includes(key.toLowerCase())
  );
